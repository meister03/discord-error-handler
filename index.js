...
class handling {
  ...
  */
  ...
  async createrr(client, guildId, msg, error) {
    if (!client) throw new TypeError("An client was not provided.");
    if (!error) throw new TypeError("An error was not provided.");
    if (!this.webhook) throw new TypeError("You did not add a webhook id or a token in the options");
    ...
    let cleaned = clean(error);
      ...
      log.setTitle("New Error Encountered!")
      if (msg) {
       log.addField(`On message in ${guildId}:` ,"```" +  msg +"```" )
      }
      log.addField("Error", "```" + smaller(error.stack,800) +"```" )
      log.setColor("RED")
      log.setTimestamp();
